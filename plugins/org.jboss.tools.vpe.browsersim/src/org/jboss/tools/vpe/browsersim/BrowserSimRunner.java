...
public class BrowserSimRunner {
	...
	public static final String NO_WEB_ENGINES = "To use BrowserSim please install Safari or run BrowserSim with Oracle Java 7u7 and higher."; //$NON-NLS-1$
	...
	static { 
		...
		if (isWindows && !BrowserSimUtil.isWindowsWebkitInstalled()) {
			isWebKitAvailable = false;
		}
		
		if (!isWebKitAvailable && !isJavaFxAvailable) {
			throw new SWTError(NO_WEB_ENGINES);
		}
	}
